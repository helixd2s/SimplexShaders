...
void main() {
    ...
    vec3 sceneNormal   = normalize(unpack2x3(texelFetch(colortex1, itexcoord, 0).xyz)[0]*2.0-1.0); 	//get the normals from the buffer we wrote them to
    vec2 sceneLightmap = unpack2x3(texelFetch(colortex2, itexcoord, 0).xyz)[1].xy; 	//Get the lightmap from the buffer we previously wrote it to in gbuffers
    ...
    if (isTerrain) {
        ...
        ctex0g[0] *= lightcolor; 		//apply lighting to diffuse
    }*/
